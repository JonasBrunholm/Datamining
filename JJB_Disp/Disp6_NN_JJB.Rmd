...
1. For a set of $X_{n,d_{(0)}}$ and $Y_{n,d_{(L)}}$.

1. Specify the step length, $\eta$, activation function $f$, and a stopping criteria.

1. Assign each weight $w_{i,j}^{(i)}$ a real value (this is often done with a small random value).

1. While stopping criteria not met

    + For $s$ in $1:n$  
    + Calculate $z_{k}^{(i)}$ and $a_{k}^{(i)}$ for $i$ in $1:L$ and $k$ in $1:d_{(i)}$.
    + Calculate $\delta_{j}^{(i)}$ for $i$ in $L:1$ and $j$ in $1:d_{(i)}$.
    + Update $w_{j,k}^{(i)} \leftarrow w_{j,k}^{(i)} - \eta \frac{1}{2n} \sum_{s=1}^n                                  \delta_{j}^{(i)}a_k^{(i-1)}$ for $i$ in $1:L$ and $k$ in $1:d_{(i)}$ and $j$ in $0:d_{(i-1)}$.
...
(test$Price - compute(nn, test[,-1])$net.result %>% as.numeric) %>% mean %>% round(5)
