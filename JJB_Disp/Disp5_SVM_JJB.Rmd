...
\begin{align}
    min_x ||x-x_i||^2, \quad \text{subject to }  \beta_0 + XB = 0,
\end{align}

from which we can construct the Lagrange

\begin{align}
    \mathcal{L}=||x-x_i||^2 + 2\lambda, (\beta_0+XB)
\end{align}
...
\begin{align}
     B^TB = 1, \: \text{and }  y_i \cdot (\beta_0 +\beta_1 x_{1,i} + \cdots + \beta_px_{p,i}) \geq M ||B||\: \forall i.
\end{align}
...
The problem posed by the maximal margin classifier is a convex optimization problem, quadratic criterion with linear inequality constraints. The solution to this problem involves constructing and solving a Lagrange function that gives rise to the following conditions
